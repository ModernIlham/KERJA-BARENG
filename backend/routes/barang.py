...
import io
import math
import html # Import HTML escaping for ReportLab
...
async def download_barang_pdf(
    ...
    """
    Robust PDF Download with HTML Escaping
    """
    query = {}
    ...
    for item in items:
        try:
            ...
            if gol != current_gol:
                ...
                # Escape Group Header
                elements.append(Paragraph(f"<b>{html.escape(gol)}</b>", group_style))
                table_data.append(headers)
                current_gol = gol
                
            # Escape Cell Data (CRITICAL for PDF crash prevention)
            ...
            row = [
                ...
                Paragraph(html.escape(kode_barang), cell_style),
                Paragraph(html.escape(nup), cell_style),
                Paragraph(html.escape(nama_barang), cell_style),
                Paragraph(html.escape(merk_tipe_str), cell_style),
                Paragraph(html.escape(kondisi), cell_style),
...
async def bulk_delete_barang(ids: Optional[List[str]] = Body(default=None), select_all_mode: bool = Body(default=False), search: Optional[str] = Body(default=None), filters: Optional[Dict[str, Any]] = Body(default=None), current_user: str = Depends(get_current_user)):
    query = {}
    if select_all_mode:
