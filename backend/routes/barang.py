...
# ... (Helpers clean_currency, clean_code_str, get_golongan_uraian same as before) ...
...
async def create_barang(barang_in: BarangCreate, current_user: str = Depends(get_current_user)):
    ...
    new_data['source'] = 'manual' # Flag as Manual
...
# ... (Include get, put, delete, pdf, export endpoints as is) ...
    ...
    for item in items:
        try:
            ...
            if gol != current_gol:
                ...
                elements.append(Paragraph(f"<b>{html.escape(gol)}</b>", group_style)); table_data.append(headers); current_gol = gol
            ...
            row = [
                ...
                Paragraph(html.escape(item.get('kode_barang','')), cell_style),
                Paragraph(html.escape(item.get('nup','')), cell_style),
                Paragraph(html.escape(item.get('nama_barang','')), cell_style),
                Paragraph(html.escape(merk_tipe_str), cell_style),
                Paragraph(html.escape(item.get('kondisi','')), cell_style),
